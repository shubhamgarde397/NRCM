...
saveDoc(i,j,data){
  ...
  DownloadLR(i,j) {
    ...
    var doc = new jsPDF({
     ...
     let data={
      ...
      'flexid':temp.flexid.length==0?'0':temp.flexid.filter(t=>{return t.village===temp.destination})[0]['flexid'],
  ...
  // Box on the page
    ...
    else if(data.partyType==='NR'){
      doc.text('NITIN ROADWAYS',80, y+26)
    }
  ...
  doc.text('PAN : AFGPG3657L',222,47)
    ...
    doc.text('PAN : AFGPG0575D',222,43)
  }
  doc.setTextColor(52,65,124)
  doc.text('DATE',222,20)
  doc.text('LR NO',262,20)
  doc.setTextColor(0,0,0)
  doc.setFontType('normal');
  doc.setFontSize('12')
  doc.setFontType('bold')
  // doc.text(data.lrshort+'-'+data.billno.split('_')[1],262,28)
  if(this.selectedDrop == 'lrno'){
    doc.text(String(data.lrno),262,28)//Change here
  }
  if(this.selectedDrop == 'billno'){
    doc.text(data.billno.split('_')[1],262,28)//Change here
  }
  ...
  doc.text('FOR SHRI NITIN LOGISTICS',222,205)
    ...
    doc.text('ISSUE CHEQUE IN FAVOR OF NITIN ROADWAYS ONLY',15,206)
    doc.setTextColor(52,65,124)
  doc.text('Signature of Consignor',145,206)
  doc.setTextColor(255,0,0)
  doc.text('FOR NITIN ROADWAYS',222,205)
